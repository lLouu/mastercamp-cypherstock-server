// QRCode from https://github.com/davidshimjs/qrcodejs/blob/master/qrcode.min.js
...
const insert_checkbox = async(id, name) => {
    let ret = document.createElement("div");
    ret.setAttribute('class', "flex row check");
...
const set_sign_up = async() => {
    ...
    flush_inputs().then(() => {
        ...
        insert_checkbox("FA", "Double Authentification");
    });
};
...
const gen_secret = async() => {
    let seed = window.crypto.getRandomValues(new Uint8Array(16));
    let ret = "";
    let choose = "ABCDEFGHIJKLMNOPQRSTUVWXYZ234567"
    for(let i = 0; i < 8; i++){
        ret += choose[Math.floor(seed[i]/8)];
    }
    return ret;
}
...
const sign_up = async() => {
    ...
    let FA = document.getElementById("FA").checked;
            ...
            res.text().then(res => {
                if(res == "success"){
                    ...
                    if(FA){
                        ...
                        instructions.textContent = "Scannez le QR code sur votre application de double authentification";
                        ...
                        insert_button("Confirmer", null, false, "FA();");
                        insert_button("Continuer sans", "/");
                    }
                    else{
...
// general backend
const login = async(epkey = null) => {
    let id = document.getElementById("username").value;
    if(!epkey){epkey = window.localStorage.getItem(id + "epkey");}
    if(!epkey){
        ...
        if(FA == null){
            ...
            let instructions = document.createElement("p");
            instructions.innerHTML = "Vous n'avez pas vos donnees encryptees en local<br>Si vous avez active l'Authentification 2 facteurs, retrouvez les donnees en indiquant votre token<br>Sinon, retrouvez votre compte via votre seed par l'inscription.";
            inputs.appendChild(instructions);
            insert_input("A2F", "FA", "text", num);
            insert_checkbox("memory", "Se souvenir des donnees");
            ...
            send_request(api_url + "/?command=0&fa=" + FA.value + "&id=" + id).then(res => {
                if(res.ok){
                    res.text().then(res => {
                        if(document.getElementById("memory").checked){window.localStorage.setItem(id + "epkey", res);}
                        login(res);
                    });
                }
            });
    ...
    gen_token(id, epkey, pwd).then(token => {
        ...
        wrong_entry(document.getElementById("password"));
    });
};
